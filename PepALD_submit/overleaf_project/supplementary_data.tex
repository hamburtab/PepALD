...
Stage & Settings \\
\midrule
Shared architecture & Uni-Mol/latent dimension 512; Transformer hidden size 512; 8 attention heads; 6 causal-context layers; 6 denoiser layers; feed-forward dimension 2,048; maximum sequence length 45; dropout 0.3; 1,000 diffusion steps with a linear variance schedule from \(\beta_1=10^{-4}\) to \(\beta_K=0.02\); R-group dimension 512; ring-predictor hidden size 256; four predicted ring-link types. \\
...
Shared Uni-Dock settings & \multicolumn{2}{>{\raggedright\arraybackslash}p{0.66\textwidth}}{Flexible ligand docking; \texttt{unidock} binary; GPU workers 0--3; batch size 1,024; ligand-preparation workers 20; scoring function \texttt{vina}; search mode \texttt{balance}; configured exhaustiveness 8; one pose per ligand; refine step 3; maximum step 20; seed 42; verbosity 0.} \\
...
For structure-based similarity metrics, each unique valid molecule is represented by a 2,048-bit Morgan fingerprint with radius 3.
